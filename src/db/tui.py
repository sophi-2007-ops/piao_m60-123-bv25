...
class Interface:
    ...
    def __init__(self) -> None:
        ...
        if choice == "2":
            self.database = FileDatabase_JSON()
        elif choice == "3":
            self.database = FileDatabase_CSV()
        else:
    ...
    def _show_all_data(self) -> None:
        print("\nСписок записей:")
        self._print_records(self.database.select_records("people"))
    ...
    def _find_person_by_filter(self) -> None:
        ...
        records = self.database.select_records("people", **filters)
        self._print_records(records)

    def _update_record_with_id(self) -> None:
        print("\nОбновление записи")
        person_id_for_update = self._read_int("Введите id записи для обновления ")
        exists = self.database.select_records(
            "people", id=person_id_for_update
        )
        if not exists:
        ...
        if updates:
            try:
                self.database.update_record_with_id(
                    "people", person_id_for_update, **updates
                )
                print("Запись обновлена")
            except (ValueError, InvalidAgeError, InvalidPhoneError) as exc:
    ...
    def _delete_record(self) -> None:
        ...
        if self.database.delete_record("people", person_id_for_delete):
    ...
    def _clear_all_data(self) -> None:
        ...
        if confirm == "да":
            if self.database.clear_all_table("people"):
